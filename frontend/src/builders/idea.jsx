...
var IDEA = {
    ...
    build(theme) {
        var xml =
            Array.prototype.concat.call(
                [
                    ...
                    f('<scheme name="{0}" version="1" parent_scheme="Default">', theme.title),
                ...
                Object.keys(theme.styles).
                    ...
                    map(name => f('\t\t<option name="{0}" value="{1}" />', name, theme.styles[name].color)),
                [
                    '\t</colors>',
                    '\t<attributes>'
                ],
                Object.keys(theme.styles).
                    ...
                    map(name => {
                        ...
                        return Array.prototype.concat.call(
                            [
                                f('\t\t<option name="{0}">', name),
                                '\t\t\t<value>'
                            ],
                            Object.keys(options).
                                filter(option => style[option]).
                                map(option => f('\t\t\t\t<option name="{0}" value="{1}" />', options[option], style[option])),
                            [
                                '\t\t\t</value>',
                                '\t\t</option>'
                            ]
        ...
        var descriptor = options = [
            ...
            f('\t\t<global_color_scheme name="{0}" />', theme.title),
            '\t</component>',
            '</application>'].join("\n");
        ...
        zip.file(f("colors/{0}.xml", safe(theme.title)), xml);

        var archive = zip.generate({type:"blob", mimeType: "application/x-zip-compressed", compression: "DEFLATE"});
